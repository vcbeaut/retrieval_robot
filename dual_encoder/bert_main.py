...
def evaluate(args, eval_dataset, model, tokenizer):
	...
	for batch in tqdm(eval_dataloader, desc="Evaluating"):
		...
		with torch.no_grad():
			...
			num_corrects += torch.sum(pos_score > neg_score).item()
			total_counts += args.eval_batch_size
	
	print('loss is: {}, accuracy is: {}'.format(eval_loss / global_step, num_corrects / total_counts))
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
	...
	if os.path.exists(cached_features_file):
		...
		examples = processor.get_dev_examples(args.dev_file) if evaluate else processor.get_train_examples(args.train_file)
...
def main():
	...
	if args.do_eval:
		...
		evaluate(args, dev_dataset, model, tokenizer)
